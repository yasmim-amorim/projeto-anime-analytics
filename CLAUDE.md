...
- **Limitação conhecida (Power BI ainda não trata multi-coleta)**: as medidas base do dashboard (`Nota Media`, `Total de Membros`, `Total de Favoritos`, etc.) somam/mediam direto sobre `fato_anime_metricas`, não sobre `vw_metricas_atuais` (a view que já existe em `sql/queries_analiticas.sql` e pega só a linha mais recente por `anime_id`). Enquanto só existir uma `data_coleta` no banco isso não tem efeito nenhum — mas se a coleta rodar de novo e uma segunda `data_coleta` for carregada, essas medidas passam a somar/mediar as duas coletas juntas, inflando os números silenciosamente. Se algum dia recoletar, reapontar essas medidas pra `vw_metricas_atuais` antes de atualizar o Power BI.
